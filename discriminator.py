...
from tensorflow.python.keras.legacy_tf_layers.core import fully_connected, flatten
...
def discriminator(self, wave_in, reuse=False):
    ...
    with tf.compat.v1.variable_scope('d_model') as scope:
        ...
        d_logit_out = fully_connected(d_logit_out, 1)

        if not reuse:
            print('discriminator output shape: ', "deneme")
            print('*****************************')
        return d_logit_out
